 ...
 *	struct ias_surface *ias_surface - a pointer to IAS surface
 ...
 * Parameters
 *	void *data - A pointer to the disp_wl class
 *	struct xdg_surface *surface - a pointer to shell's surface
 *	uint32_t serial - serial of configure event
 * Return val
 *	void
 ******************************************************************************/
void
disp_wl::handle_surface_configure(void *data, struct xdg_surface *surface,
			 uint32_t serial)
{
	disp_wl *d = (disp_wl *) data;
 ...
 * Parameters
 *	void *data - A pointer to the disp_wl class
 *	struct xdg_toplevel *toplevel- A pointer to the xdg_toplevel structure
 *	int32_t width - The new width
 *	int32_t height - The new height
 ...
 *	struct xdg_toplevel *xdg_toplevel - A pointer to xdg_toplevel struct
 * Return val
 *	void
 ******************************************************************************/
 ...
 *	are multiple shells (ias/xdg), it checks to see which one did this
...
EGLNativeWindowType disp_wl::create_surface(int width, int height, const char* title)
{
	...
	if (g->wm_base) {
		s.shell_surface = xdg_wm_base_get_xdg_surface(
				(struct xdg_wm_base *) g->wm_base, s.surface);
		xdg_surface_add_listener((struct xdg_surface *) s.shell_surface,
				&xdg_surface_listener, this);
		s.xdg_toplevel =
				xdg_surface_get_toplevel(
					(struct xdg_surface *) s.shell_surface);
		xdg_toplevel_add_listener(s.xdg_toplevel,
...
void disp_wl::destroy_surface()
{
	...
	if (g->wm_base)
		xdg_surface_destroy((xdg_surface *) s.shell_surface);
	if (g->ias_shell) {
